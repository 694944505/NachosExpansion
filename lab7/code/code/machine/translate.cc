...
bool Machine::ReadMem(int addr, int size, int *value)
{
	...
	DEBUG('a', "Reading VA 0x%x, size %d, vpn %d\n", addr, size, addr / PageSize);
	//printf("11112\n");
...
Machine::Translate(int virtAddr, int *physAddr, int size, bool writing)
{
	...
	if (((size == 4) && (virtAddr & 0x3)) || ((size == 2) && (virtAddr & 0x1)))
	{
		...
		DEBUG('I', "virtual page # %d too large for page table size %d!\n",
	...
	{ // => page table => vpn is index into table
		if (!useInvertedPageTable)
		{
			...
			if(i==NumPhysPages) return PageFaultException;
		}/*
		if (vpn >= pageTableSize)
			{
				DEBUG('a', "virtual page # %d too large for page table size %d!\n",
		...
		if (!useInvertedPageTable)
		{
			for (entry = NULL, i = 0; i < TLBSize; i++)
				if (tlb[i].valid && (tlb[i].virtualPage == vpn))
				{
					tlbHit++;
					entry = &tlb[i]; // FOUND!
					tlb[i].lastUseTime = stats->userTicks;
					break;
				}
			if (entry == NULL)
			{ // not found
				...
				tlbMiss++;
				tlbHit--;				   //the next Translate will add 1 to tlbHit
			...
			for (entry = NULL, i = 0; i < TLBSize; i++)
				if (tlb[i].valid && (tlb[i].virtualPage == vpn)
				&&tlb[i].threadID==currentThread->getThreadID())
				{
					tlbHit++;
					entry = &tlb[i]; // FOUND!
					tlb[i].lastUseTime = stats->userTicks;
