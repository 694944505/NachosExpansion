...
void Machine::SwapTLB(int vpn)
{
    ...
    default:
        printf("Wrong SwapType:%s\n",swapType);
        ASSERT(FALSE);
        break;
    }
}

void Machine::SwapTLB_FIFO(int vpn){
    if(!useInvertedPageTable){
        for(int i=0;i<TLBSize;i++){
            if(!tlb[i].valid){
                tlb[i]=pageTable[vpn];
                tlb[i].firstUseTime=stats->userTicks;
                return ;
            }
        }
        int i=0;
        for(int j=1;j<TLBSize;j++){
            if(tlb[i].firstUseTime>tlb[j].firstUseTime){
                i=j;
            }
        }
        tlb[i]=pageTable[vpn];
...
void Machine::SwapTLB_LRU(int vpn){
    if(!useInvertedPageTable){
        for(int i=0;i<TLBSize;i++){
            if(!tlb[i].valid){
                tlb[i]=pageTable[vpn];
        ...
        int i=0;
        for(int j=1;j<TLBSize;j++){
            if(tlb[i].lastUseTime>tlb[j].lastUseTime){
                i=j;
            }
        }
        tlb[i]=pageTable[vpn];
...
int Machine::allocMemory(){
    int i=bitMap->Find();
    if(multiThread)printf("***allocate memory %d\n",i);
    return i;
}

void Machine::freeMemory(){
    for(int i=0;i<pageTableSize;i++){
        if(multiThread)printf("***deallocate memory %d\n",pageTable[i].physicalPage);
