...
{
    //printf("Createing %s %d\n",name,currentDirectoryFile->oSector);
    if(name[0]=='/') Create(name, initialSize, directoryFile);
    else Create(name, initialSize, currentDirectoryFile);
}
...
{
    if(name[0]=='/') return Open(name, directoryFile);
    else return Open(name, currentDirectoryFile);
}
...
{
    if(name[0]=='/') return Remove(name, directoryFile);
    else return Remove(name, currentDirectoryFile);
}
...
{
    if(name[0]=='/') return List(name, directoryFile);
    else return List(name, currentDirectoryFile);
}
...
bool FileSystem::Mkdir(char *name, int initialSize, OpenFile *direcFile)
{
    ...
    if (sector != -1)
    {
        ...
        {
            name[i] = '/';
            return Mkdir(name + i + 1, initialSize, new OpenFile(sector));
            ;
        }
    }
    else
        ...
        else
        {
...

            {
                ...
                else
                {
                   ...
                   // mydirectory->FetchFrom(opp);
                   // mydirectory->List();
                    success = Mkdir(name + i + 1, initialSize, opp);
                    name[i] = '/';
                }
            }
            delete hdr;
